...
const resolverCaptcha = async () => {
  ...
  try {
    ...
    logger.error(`Can not resolve captcha, error: ${error.message}, ${error.stack}`);
    return null;
  }
}
...
const parseFinesUrl = async (finesUrl, sessionId) => {
  ...
  const KEYS = ['plate', 'plateColor', 'vehicleType', 'violationTime', 'location', 'violationType', 'status', 'detectingUnit'];
